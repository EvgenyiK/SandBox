...
DEFINE_LOG_CATEGORY_STATIC(LogHubGeometry, All, All)
...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World)
	{
		for (int32 i = 0; i < 10; ++i) {
			...
			auto Geometry = World->SpawnActor<ABaseGeometryActor>(GeometryClass, GeometryTransform);
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World)
	{
		for (int32 i = 0; i < 10; ++i) {
			...
			auto Geometry = World->SpawnActorDeferred<ABaseGeometryActor>(GeometryClass, GeometryTransform);
...
void AGeometryHubActor::DoActorSpawn3()
{
	...
	if (World)
	{
		...
		{
			auto* Geometry = World->SpawnActorDeferred<ABaseGeometryActor>(Payload.GeometryClass, Payload.InitialTransform);
...
{
	UE_LOG(LogHubGeometry, Warning, TEXT("Actor name: %s Color %s"), *Name, *Color.ToString());
}
...
void AGeometryHubActor::OnTimerFinished(AActor* Actor)
{
	...
	UE_LOG(LogHubGeometry, Error, TEXT("Timer finished %s)"), *Actor->GetName());

	ABaseGeometryActor* Geometry = Cast<ABaseGeometryActor>(Actor);
	if (!Geometry) { return; }
	UE_LOG(LogHubGeometry, Display, TEXT("Cast is success, amplitude %f"), Geometry->GetGeometryData().Amplitude);
